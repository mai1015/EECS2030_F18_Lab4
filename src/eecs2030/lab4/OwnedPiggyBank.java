...
public class OwnedPiggyBank {
	...
	public void changeOwner(Owner currentOwner, Owner newOwner) {
		if (!currentOwner.equals(owner)) throw new IllegalArgumentException("Illegal access to change owner");
		this.owner = newOwner;
	}
	...
	public List<Coin> removeCoins(Owner user, int value) {
		...
		if (!user.equals(owner)) return list;

		Collections.sort(coins, Collections.reverseOrder());
		List<Coin> copy = deepCopy();
		int current = value;
		for (int i = 0; i < copy.size() && current > 0; i++) {
			if (copy.get(i).getValue() <= current) {
				current -= copy.get(i).getValue();
